class Solution {
    public List<List<String>> groupAnagrams(String[] strs) {
        ...
        for (int i = 0; i < len; i++) { // traverse thru keys
            if (keys[i] != null) { // if key exists,
                List<String> key = keys[i]; // get the key.
                List<String> words = anagrams.get(key); // get the anagram list that corresponds,
                answer.add(words); // and add it to the answer!
            }
        }
        return answer;
    }
